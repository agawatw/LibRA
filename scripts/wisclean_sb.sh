...
then
    ...
    # echo "MODEL_DATA init..."
    # echo -e "load psf_predict_init.def \ngo" | ${griddingAPP} help=dbg 2>| model_predict_init.log
    # ${griddingAPP} help=def,psf_predict_init.def  2>| ${logdir}/model_predict_init.log
    
    # generate weight
    echo ${RUNAPP} ${griddingAPP} ${MODE_WEIGHT} ${imagename} ${input_file} ${logdir}   
    ${RUNAPP} ${griddingAPP} ${MODE_WEIGHT} ${imagename} ${input_file} ${logdir}   

    mv ${imagename}.${MODE_WEIGHT} ${imagename}.weight
    # save the gridded CF
    cp -r ${imagename}_gridv.vis ${imagename}_gridcf.vis0

    # # generare residual
    echo ${RUNAPP} ${griddingAPP} ${MODE_RESIDUAL} ${imagename} ${input_file} ${logdir} -c 0
    ${RUNAPP} ${griddingAPP} ${MODE_RESIDUAL} ${imagename} ${input_file} ${logdir} -c 0

    mv ${imagename}.${MODE_RESIDUAL} ${imagename}.residual
    # # save the gridded vis
    cp -r ${imagename}_gridv.vis ${imagename}_gridres.vis0

    # # generare psf
    echo ${RUNAPP} ${griddingAPP} ${MODE_PSF} ${imagename} ${input_file} ${logdir} -c 0
    ${RUNAPP} ${griddingAPP} ${MODE_PSF} ${imagename} ${input_file} ${logdir} -c 0

    mv ${imagename}.${MODE_PSF} ${imagename}.psf
    # cp -r ${imagename}.residual ${imagename}_beforeNorm.dirty.cycle0
    # cp -r ${imagename}.psf ${imagename}_beforeNorm.psf.cycle0

    # # save the gridded vis
    cp -r ${imagename}_gridv.vis ${imagename}_gridpsf.vis0
...
do
    ...
    # save model at each cycle for debugging
    ...
    # normalize the new residual and PSF
